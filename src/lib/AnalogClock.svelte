...
<div class="relative aspect-square w-full max-w-xl mx-auto overflow-hidden">
  {#each {length: 60} as _, d}
    <div class="w-full h-full absolute 0" style={`transform: rotate(${d * 6}deg);`}>
      {#if d % 5 == 0}
      <div class="w-[calc(50%+2px)] h-5 border-r-4 border-white" />
      {:else}
      <div class="w-[calc(50%+1px)] h-1 border-r-2 border-white" />
      {/if}
    </div>
  {/each}
  <div class="w-2/3 h-2/3 absolute top-[calc(100%/6)] left-[calc(100%/6)]" style={`transform: rotate(${hDeg}deg);`}>
    <div class="w-[calc(50%+4px)] h-1/2 border-r-8 border-white" />
  </div>
  <div class="w-full h-full absolute top-0" style={`transform: rotate(${mDeg}deg);`}>
    <div class="w-[calc(50%+2px)] h-1/2 border-r-4 border-white" />
  </div>
